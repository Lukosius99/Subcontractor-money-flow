...
3. PAD parsisiunčia laikiną failą ir siunčia `POST /api/imports/monthly-flow?sourceFileName=...`.
...
PAD siunčia `POST /api/imports/contracts` su bent vienu masyvu: `rows` arba `projectValueRows`. Sutarties eilutei būtini projekto kodas, objekto numeris, subrangovas ir sutartinė suma; projekto vertės eilutei — projekto kodas, objekto numeris ir vertė.

Importas yra naujausio snapshot pobūdžio: sistemoje pažymima, kurie projektai / kontraktai buvo matyti naujausiame importe. Prieš keičiant šią logiką būtina paleisti `contract-snapshot.ps1`.
